...
QString error[] = {
    "Неверное имя пользователя и/или пароль. Пожалуйста, попробуйте снова.",
    ...
    "Имя пользователя не может содержать пробелы. Пожалуйста, попробуйте снова.",
    "Пользователь уже вошел в систему.",
    "Произошла ошибка. Пожалуйста, перезапустите программу."
};
...
void Window::on_login_button_clicked()
{
    ...
    if (connection->get_status())
    {
        ...
        else
        {
            ...
            signal += ' ';
            signal += user.get_user_id();  // Если get_user_id() возвращает QByteArray, преобразование не требуется
            signal += ' ';
            signal += username_check.toUtf8();
            signal += ' ';
            signal += password_check.toUtf8();
...
void Window::on_send_button_clicked()
{
    ...
    if (connection->get_status())
    {
        ...
        {
            QChar currentChar = message[i];

            if (currentChar.unicode() > 31)
            {
                new_message += currentChar;

                if (currentChar.unicode() > 32)
...
void Window::on_receive_message(QByteArray sender, QByteArray destination, QString message)
{
    ...
    if (connection->get_destination() != sender)
        color_notify(sender);
    qDebug() << "color_notify done";

    QString userStatus;
    if (chat_map.contains(destination)) {
        // Получаем указатель на объект Contact
        Contact* contact = chat_map[destination];

        // проверяем, что указатель не равен nullptr
        if (contact) {
            // получаем текст из status_label
            QString userStatus = contact->status_label->text();

            // используем userStatus
            qDebug() << "User status:" << userStatus;

        } else {
            qDebug() << "Contact is nullptr for destination:" << destination;
        }
    } else {
        qDebug() << "Destination not found in chat_map:" << destination;
    }
...
void Window::color_notify(QByteArray sender)
{
    for (int i = 0; i < ui.contacts->count(); i++)
    {
        ...
        if (item->text() == sender)
        {
            QWidget *existingWidget = ui.contacts->itemWidget(item);
            if (existingWidget)
            {
                // Удаляем предыдущий виджет, если он был установлен
                ui.contacts->removeItemWidget(item);
                delete existingWidget;
            }

            QLabel *label = new QLabel(sender);
            label->setStyleSheet("background-color: rgb(42, 42, 76); color: rgb(255, 255, 255);");

            ui.contacts->setItemWidget(item, label);
        }
    }
    qDebug() << "color_notify";
}
...
void Window::format_output(QString mode, QString contact, QByteArray name, QString message)
{
    ...
    if (mode == "message")
    {
        ...
        for (int i = 0; i < message.length(); i++)
        {
            if (message.at(i).unicode() > 32)
            {
                new_message.append(QByteArray(1, message.at(i).toLatin1()));
                line_break_count++;
            }
            else if (message.at(i) == QChar(' '))
            {
                new_message.append(QByteArray(1, message.at(i).toLatin1()));
                line_break_count = 0;
            }
    ...
    if (name == user.get_username())
    {
        ...
        if (*last_message != "user")
        {
            ...
            QChar bubble_letter(QString(user.get_username()).at(0).toUpper());
    ...
    else
    {
        ...
        if (*last_message != "contact")
        {
            ...
            QChar bubble_letter(contact.at(0).toUpper());
            bubble->setText(bubble_letter);
        }
...
void Window::on_add_line_textChanged(const QString &arg1)
{
    ...
    if (connection->get_status())
    {
        if (!ui.add_line->text().isEmpty())
        {
            ...
            QByteArray signal = QByteArray(ADD_CONTACT) + ' '
                                + username.toLatin1() + ' '
                                + ui.add_line->text().toLatin1();
...
void Window::on_contacts_itemSelectionChanged()
{
    ...
    if (!delete_mode)
    {
        ...
        if (itr != chat_map.end())
        {
            ...
            // установка стиля через CSS для выделенного элемента
            QString style = "QListWidget::item:selected { background-color: rgb(199, 199, 216); color: rgb(0, 0, 0); }";
            ui.contacts->setStyleSheet(style);
...
void Window::load_log(QString contact)
{
    ...
    if (itr == chat_map.end())
    {
        ...
        if (query.value(0).toInt() == 0)
        {
            ...
            signal += ' ';
            signal += username.toUtf8();
            signal += ' ';
            signal += contact.toUtf8();
        ...
        else
        {
            ...
            signal += contact.toUtf8();
            signal += ' ';
            signal += query.value(0).toByteArray();
...
void Window::on_update_log(QString contact)
{
    emit(lock());
    if (!chat_map.contains(contact)) {
        qDebug() << "Contact not found in chat_map: " << contact;
        emit(unlock());
        return;
    }
    QVBoxLayout *current_window = chat_map[contact]->layout;
    ...
    QSqlQuery query;
    QByteArray username = user.get_username(); // Преобразуем QString в QByteArray с кодировкой UTF-8

    QString queryString = "select distinct first_participant, second_participant, "
                          "message, timestamp from messages where (first_participant = '" +
                          username + "' and second_participant = '" + contact + "') or "
                                                                                "(first_participant = '" + contact + "' and second_participant = '" +
                          username + "') order by timestamp;";

    query.exec(queryString);

    int firstParticipantIndex = query.record().indexOf("first_participant");
    int messageIndex = query.record().indexOf("message");

    while (query.next())
    {
        QByteArray firstParticipant = query.value(firstParticipantIndex).toByteArray();
        QByteArray message = query.value(messageIndex).toByteArray();

        if (message.contains(PHOTO))
            format_output("photo", contact, firstParticipant, message);
        else
            format_output("message", contact, firstParticipant, message);
    }
